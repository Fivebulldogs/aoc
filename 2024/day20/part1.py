...
for i, pos in enumerate(original_path):
    ...
    nearby_obstacles = get_nearby_obstacles(pos, obstacles, width, height)

    for obstacle_to_remove in nearby_obstacles:
        if obstacle_to_remove in visited_obstacles:
            continue
        costs = get_costs(original_costs, obstacle_to_remove)
        prev = find_path(pos, end, obstacles, obstacle_to_remove, costs, width, height)
        path = get_path(pos, end, prev)
        path_time = len(path) + i
        if path_time < original_time:
            cheats[path_time] += 1
        visited_obstacles.add(obstacle_to_remove)
...
for k, v in sorted(time_saved.items()):
    ...
    print(f"{v} cheats save {k+1} picoseconds.")
