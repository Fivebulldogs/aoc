...
def add_metadata(node, i, metadata_sum):
    ...
    metadata_sum += sum(node.metadata_entries)
    return (node.end_index, metadata_sum)
...
try:
    while True:
        ...
        if curr_node.header.child_nodes_qty > 0:
            if curr_node.children_processed < curr_node.header.child_nodes_qty:
                ...
                (i, metadata_sum) = add_metadata(curr_node, i, metadata_sum)
                print(
    ...
    print(f"All done! metadata_sum: {metadata_sum}")
